...
from Game.Utility.Modifiers.GravityMod import GravityMod
from Game.Utility.Modifiers.BounceMod import BounceMod
...
class MoonsMid(MinorArcanaType):
    ...
    def __init__(self, gc: GameContainer, pos: v.Vector = v.Vector(0, 0), power: int = 5, image: str = "Five"):
        ...
        sprite = gc.ImageManager.get_image("Moons/"+image, scale=(2.5, 2.5))
    ...
    def update(self, gc: GameContainer, dt: float, dt_normalized: float) -> None:
        ...
        if self.animation.trigger_at("Attack", 17):
            pattern = DelayedTargeting(gc, v.duplicate(self.pos), 2, self.power, 0.5)
            match self.power:
                case 5:
                    pattern.add_modifier(GravityMod().initialize(150, 6))
                case 6:
                    pattern.add_modifier(RewindMod())
                case 7:
                    pattern.add_modifier(SplitMod().initialize(4)). \
                            add_modifier(BounceMod())
                case 8:
                    pattern.add_modifier(GravityMod().initialize(150, 6)). \
                            add_modifier(ExplodingMod())
